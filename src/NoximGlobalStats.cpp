...
void NoximGlobalStats::showStats(std::ostream & out, bool detailed)
{
    ...
    //out << "% Total received packets: " << ReceiveNumber<<endl;//getReceivedPackets() << endl;
    //out << "percentage of sent and received" << percent<<"%"<<endl;
    //for(int i=0;i<PacketState.size();i++){out<<"########STATE"<<i<<":"<<PacketState[i]<<endl;}
